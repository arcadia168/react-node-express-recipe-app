...
* Updated `cssstyle` minimum version to ensure all jsdom installs (not just fresh ones) get the ***REMOVED***efit of `cssstyle`'s recently-better `background` and `width` setters.
...
However, recent performance investigations revealed that mutation events were the major bottleneck in most jsdom operations; tools like [ecmarkup](https://github.com/bterlson/ecmarkup) which make heavy use of jsdom had their running time halved by removing mutation events, which add serious overhead to every DOM mutation. As such, we are doing a major release with them removed, so that jsdom users can ***REMOVED***efit from this massive performance gain.
...
* Updated `cssstyle` minimum version to ensure all jsdom installs (not just fresh ones) get the ***REMOVED***efit of `cssstyle`'s recently-better `padding` and `margin` parsing/CSSOM.
...
Additionally, Joris-van-der-Wel added [a ***REMOVED***chmarking framework](https://github.com/tmpvar/jsdom/blob/master/Contributing.md#running-the-***REMOVED***chmarks), and a number of ***REMOVED***chmarks, which should help us avoid performance regressions going forward, and also make targeted performance fixes. We're already investigating [some real-world issues](https://github.com/tmpvar/jsdom/issues/1156) using this framework. Very exciting!
