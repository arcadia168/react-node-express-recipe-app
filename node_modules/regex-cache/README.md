...
* See [the ***REMOVED***chmarks](#***REMOVED***chmarks)
...
* **A few options are passed**, and the values are primitives. The limited ***REMOVED***chmarks I did show that caching is ***REMOVED***eficial when up to 8 or 9 options are passed.
...
```bash
npm i -d && npm run ***REMOVED***chmarks
```
