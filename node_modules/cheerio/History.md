...
==================
  ...
  * Use jQuery from the jquery module in ***REMOVED***chmarks (#871)
 ...
 * #608 -- Add support for insertBefore/insertAfter syntax. Supports target types of: $, [$], selector (both single and multiple results) (***REMOVED*** Cochran)
 ...
 * fix `make ***REMOVED***ch` (David Chambers)
 ...
 * Add cheerio-only ***REMOVED***ch option (@kpdecker)
 * Avoid delete operations (@kpdecker)
 * Add independent html ***REMOVED***chmark (@kpdecker)
 * Cache tag check in render (@kpdecker)
 * Simplify attribute rendering step (@kpdecker)
 * Add html rendering ***REMOVED***ch case (@kpdecker)
 ...
 * Add single element ***REMOVED***chmark case (@kpdecker)
 ...
 * Add ***REMOVED***chmark coverage for all $ APIs (@kpdecker)
 ...
 * Introduce rudimentary ***REMOVED***chmark suite (@jugglinmike)
...
* Added a toString() method (@***REMOVED***sheldon)
* use `_.each` and `_.map` to simplify cheerio namesakes (@davidchambers)
* Added filter() with tests and updated readme (@***REMOVED***sheldon & @davidchambers)
...
* fixed xss vulnerabilities on .attr(), .text(), & .html() (@***REMOVED***atkin, @FB55)
...
* Added ***REMOVED***chmark directory for future speed tests
