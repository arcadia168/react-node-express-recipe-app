...
## 5.2.8
* Fix error message (by ***REMOVED*** Briggs).
...
## 5.2.5
* Better error message on unclosed string (by ***REMOVED*** Briggs).
...
## 5.0.17
* Fix options argument in `Node#warn` (by ***REMOVED*** Briggs).
* Fix TypeScript definitions (by Jed Mao).
...
* Deprecate `Container#remove` and `Node#removeSelf`
  in favor of `Container#removeChild` and `Node#remove` (by ***REMOVED*** Briggs).
* Deprecate `Node#replace` in favor of `replaceWith` (by ***REMOVED*** Briggs).
...
* Add `plugin.process(css)` shortcut to every plugin function (by ***REMOVED*** Briggs).
* Add multiple nodes support to insert methods (by Jonathan Neal).
* Add `Node#warn` shortcut (by ***REMOVED*** Briggs).
...
## 4.1.15
* Allow asynchronous plugins to change processor plugins list (by ***REMOVED*** Briggs).
...
* Fix warning message (by ***REMOVED*** Briggs).
...
* Parse `!important` statement with spaces and comments inside (by ***REMOVED*** Briggs).
